...
export namespace Math {
	...
	public:
		...
		double Mod() { return(sqrt(m_re * m_re + m_im * m_im)); }
		double Arg() { return(std::atan2(m_im, m_re)); }
		...
		Complex& operator--() { this->m_re = this->m_re - 1; return *this; }
		Complex operator--(int peregruz) { Complex copy = *this; this->m_re = this->m_re - 1; return copy; }
		Complex& operator+=(Complex complex) { this->m_re + complex.m_re; this->m_im + complex.m_im; return *this; }
		Complex& operator-=(Complex complex) { this->m_re - complex.m_re; this->m_im - complex.m_im; return *this; }
		Complex& operator*= (Complex x)
		{
			this->m_re = this->m_re * x.m_re - this->m_im * x.m_im;
			this->m_im = this->m_re * x.m_im + this->m_im * x.m_re;
			return *this;
		}
		Complex& operator/= (Complex x)
		{
			this->m_re = (this->m_re * x.m_re + this->m_im * x.m_im) / (x.m_re * x.m_re + x.m_im * x.m_im);
			this->m_im = (this->m_im * x.m_re - this->m_re * x.m_im) / (x.m_re * x.m_re + x.m_im * x.m_im);
			return *this;
		}
	...
	Complex tg(Complex x) {
		double a = x.Re();
		double b = x.Im();
		return Complex(((sin(2 * a)) / (cos(2 * a) + cosh(2 * b))), (sinh(2 * b) / (cos(2 * a) + cosh(2 * b))));
	}
